...
/**
 * @file BeehiveRoutingTable.cc
 * @author Grigory Chirkov
 * Based on the Pastry overlay implementation
 */

#include "BeehiveRoutingTable.h"


Define_Module(BeehiveRoutingTable);

uint32_t BeehiveRoutingTable::digitAt(uint32_t n,
...
void BeehiveRoutingTable::earlyInit(void)
{
    WATCH_VECTOR(rows);
}


void BeehiveRoutingTable::initializeTable(uint32_t bitsPerDigit,
...
const BeehiveExtendedNode& BeehiveRoutingTable::nodeAt(uint32_t row,
                                                     uint32_t col) const
{
...
const NodeHandle& BeehiveRoutingTable::lookupNextHop(const OverlayKey& destination)
{
    ...
    if (shl >= rows.size()) {
        EV << "Beehive: Unable to find next hop for " << destination
           << ", row is empty." << endl;
        return NodeHandle::UNSPECIFIED_NODE;
    }

    const BeehiveExtendedNode& next = nodeAt(shl, digit);
...
const NodeHandle& BeehiveRoutingTable::findCloserNode(const OverlayKey& destination,
                                                     bool optimize)
{
    if (destination == owner.getKey())
        opp_error("trying to find closer node to own key!");

    const BeehiveExtendedNode* entry;
...
void BeehiveRoutingTable::findCloserNodes(const OverlayKey& destination,
                                         NodeVector* nodes)
{
    //TODO
    const BeehiveExtendedNode* entry;
...
void BeehiveRoutingTable::dumpToStateMessage(BeehiveStateMessage* msg) const
{
    uint32_t i = 0;
    uint32_t size = 0;
...
void BeehiveRoutingTable::dumpRowToMessage(BeehiveStateMessage* msg,
                                          int row) const
{
    uint32_t i = 0;
    uint32_t size = 0;
...
std::vector<TransportAddress>* BeehiveRoutingTable::getRow(uint8_t row) const
{
...
int BeehiveRoutingTable::getLastRow()
{
    return rows.size();
}


const TransportAddress& BeehiveRoutingTable::getRandomNode(int row)
{
    std::vector<PRTRow>::const_iterator itRows;
    PRTRow::const_iterator itCols;
    uint32_t rnd;
    ...
    if (itRows >= rows.end()) {
        EV << "[BeehiveRoutingTable::getRandomNode()]\n"
...
bool BeehiveRoutingTable::mergeNode(const NodeHandle& node, simtime_t prox)
{
    ...
    if (position->node.isUnspecified() || (prox < position->rtt)) {
        EV << "[BeehiveRoutingTable::mergeNode()]\n"
           << "    Node " << owner.getKey()
           << endl;
...
bool BeehiveRoutingTable::initStateFromHandleVector(const std::vector<BeehiveStateMsgHandle>& handles)
{
    std::vector<BeehiveStateMsgHandle>::const_iterator it;
    int hopCheck = 0;
...
void BeehiveRoutingTable::dumpToVector(std::vector<TransportAddress>& affected) const
{
    std::vector<PRTRow>::const_iterator itRows;
    PRTRow::const_iterator itCols;
...
void BeehiveRoutingTable::addRow(void)
{
...
{
    os << "Beehive IRoutingTable row {" << endl;
...
const TransportAddress& BeehiveRoutingTable::failedNode(const TransportAddress& failed)
{
...
const TransportAddress& BeehiveRoutingTable::repair(const BeehiveStateMessage* msg,
...
void BeehiveRoutingTable::findNextNodeToAsk(PRTTrackRepair& track) const
{
