...
void Beehive::clearVectors()
{
    // purge pending state messages
    if (!stReceived.empty()) {
        for (std::vector<BeehiveStateMsgHandle>::iterator it =
...
void Beehive::purgeVectors(void)
{
    clearVectors();

    // purge vector of waiting sendState messages:
    if (! sendStateWait.empty()) {
        for (std::vector<BeehiveSendState*>::iterator it =
...
void Beehive::initializeOverlay(int stage)
{
    ...
    overrideOldBeehive = par("overrideOldBeehive");
    overrideNewBeehive = par("overrideNewBeehive");

    if (overrideOldBeehive) {
        useSecondStage = true;
        useDiscovery = false;
...
void Beehive::changeState(int toState)
{
    ...
    case DISCOVERY: {
        ...
        BeehiveRequestLeafSetCall* call =
            new BeehiveRequestLeafSetCall("REQUEST LEAFSET Call");
        call->setStatType(MAINTENANCE_STAT);
        call->setBitLength(BEEHIVEREQUESTLEAFSETCALL_L(call));
    ...
    case JOIN: {
        ...
        BeehiveJoinCall* call = new BeehiveJoinCall("JOIN Call");
        call->setStatType(MAINTENANCE_STAT);
        call->setBitLength(BEEHIVEJOINCALL_L(msg));
...
void Beehive::pingResponse(PingResponse* pingResponse,
...
void Beehive::handleTimerEvent(cMessage* msg)
{
    if (msg == readyWait) {
        if (partialJoinPath) {
            ...
            EV << "[Beehive::handleTimerEvent() @ " << thisNode.getIp()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
        ...
        if ((depth == 0) || (discoveryModeProbedNodes < 1)) {
            ...
            BeehiveRequestRoutingRowCall* call =
                new BeehiveRequestRoutingRowCall("REQUEST ROUTING ROW Call");
            call->setStatType(MAINTENANCE_STAT);
            call->setRow(depth);
            call->setBitLength(BEEHIVEREQUESTROUTINGROWCALL_L(call));
        ...
        scheduleAt(simTime() + routingTableMaintenanceInterval,
                   repairTaskTimeout);
    } else if (dynamic_cast<BeehiveSendState*>(msg)) {
        BeehiveSendState* sendStateMsg = static_cast<BeehiveSendState*>(msg);

        std::vector<BeehiveSendState*>::iterator pos =
            std::find(sendStateWait.begin(), sendStateWait.end(),
                      sendStateMsg);
        if (pos != sendStateWait.end()) sendStateWait.erase(pos);

        BeehiveStateMessage* stateMsg = createStateMessage();
...
void Beehive::sendStateDelayed(const TransportAddress& destination)
{
    BeehiveSendState* selfMsg = new BeehiveSendState("sendStateWait");
...
{
    BeehiveStateMessage* stateMsg = check_and_cast<BeehiveStateMessage*>(msg);
    uint32_t type = stateMsg->getBeehiveStateMsgType();

    if (debugOutput) {
        EV << "[Beehive::handleUDPMessage() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    incoming STATE message of type "
           << cEnum::get("BeehiveStateMsgType")->getStringFor(type) << endl;
    }
...
{
    if (BaseBeehive::handleRpcCall(msg)) return true;
    ...
    RPC_DELEGATE( BeehiveJoin, handleBeehiveJoinCall );
    RPC_DELEGATE( BeehiveRequestState, handleRequestStateCall );
    RPC_DELEGATE( BeehiveRequestRepair, handleRequestRepairCall );
    RPC_SWITCH_END( )
...
void Beehive::handleBeehiveJoinCall(BeehiveJoinCall* call)
{
    EV << "[Beehive::handleBeehiveJoinCall() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    if (state != READY) {
        if (call->getSrcNode() == thisNode) {
            EV << "[Beehive::handleBeehiveJoinCall() @ " << thisNode.getIp()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    BeehiveJoinCall received by originator!"
               << endl;
        } else {
            EV << "[Beehive::handleBeehiveJoinCall() @ " << thisNode.getIp()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
        ...
        EV << "[Beehive::handleBeehiveJoinCall() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    BeehiveJoinCall gets dropped because it is "
        ...
        BeehiveJoinResponse* response = new BeehiveJoinResponse("JOIN Response");
        ...
        response->setBitLength(BEEHIVEJOINRESPONSE_L(response));
        response->encapsulate(createStateMessage((minimalJoinState ?
                                                  BEEHIVE_STATE_MINJOIN :
                                                  BEEHIVE_STATE_JOIN),
                                                  -1, joinHopCount, true));
...
void Beehive::handleRequestStateCall(BeehiveRequestStateCall* call)
{
    EV << "[Beehive::handleRequestStateCall() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    BeehiveRequestStateResponse* response =
        new BeehiveRequestStateResponse("REQUEST STATE Response");
    response->setStatType(MAINTENANCE_STAT);

    response->setBitLength(BEEHIVEREQUESTSTATERESPONSE_L(response));
...
void Beehive::handleRequestRepairCall(BeehiveRequestRepairCall* call)
{
    EV << "[Beehive::handleRequestRepairCall() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    BeehiveRequestRepairResponse* response =
        new BeehiveRequestRepairResponse("REQUEST REPAIR Response");
    response->setStatType(MAINTENANCE_STAT);

    response->setBitLength(BEEHIVEREQUESTREPAIRRESPONSE_L(response));
    response->encapsulate(createStateMessage(BEEHIVE_STATE_REPAIR));
    RECORD_STATS(stateSent++;
                 stateBytesSent += response->getByteLength());
...
void Beehive::handleRequestRepairResponse(BeehiveRequestRepairResponse* response)
{
    EV << "[Beehive::handleRequestRepairResponse() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
...
void Beehive::handleRpcResponse(BaseResponseMessage* msg,
                               ...
                               simtime_t rtt)
{
    BaseBeehive::handleRpcResponse(msg, context, rpcId, rtt);
    ...
    RPC_ON_RESPONSE( BeehiveJoin ) {
        EV << "[Beehive::handleRpcResponse() @ " << thisNode.getIp()
           ...
           << "    msg=" << *_BeehiveJoinResponse << " rtt=" << SIMTIME_DBL(rtt)
           << endl;
        handleBeehiveJoinResponse(_BeehiveJoinResponse);
        break;
        }
    RPC_ON_RESPONSE( BeehiveRequestState ) {
        EV << "[Beehive::handleRpcResponse() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Received a RequestState RPC Response: id=" << rpcId << "\n"
           << "    msg=" << *_BeehiveRequestStateResponse << " rtt=" << SIMTIME_DBL(rtt)
           << endl;
        handleRequestStateResponse(_BeehiveRequestStateResponse);
        break;
    }
    RPC_ON_RESPONSE( BeehiveRequestRepair ) {
        EV << "[BaseBeehive::handleRpcResponse() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Received a Request Repair RPC Response: id=" << rpcId << "\n"
           << "    msg=" << *_BeehiveRequestRepairResponse << " rtt=" << SIMTIME_DBL(rtt)
           << endl;
        handleRequestRepairResponse(_BeehiveRequestRepairResponse);
        break;
    }
    RPC_ON_RESPONSE( BeehiveRequestLeafSet ) {
        EV << "[Beehive::handleRpcResponse() @ " << thisNode.getIp()
           ...
           << endl;
        handleRequestLeafSetResponse(_BeehiveRequestLeafSetResponse);
        break;
    }
    RPC_ON_RESPONSE( BeehiveRequestRoutingRow ) {
        EV << "[Beehive::handleRpcResponse() @ " << thisNode.getIp()
           ...
           << "    msg=" << *_BeehiveRequestRoutingRowResponse << " rtt=" << rtt
           << endl;
        handleRequestRoutingRowResponse(_BeehiveRequestRoutingRowResponse);
        break;
    }
    RPC_SWITCH_END( )
}
...
void Beehive::handleRpcTimeout(BaseCallMessage* call,
                              const TransportAddress& dest,
                              cPolymorphic* context, int rpcId,
                              const OverlayKey& key)
{
    BaseBeehive::handleRpcTimeout(call, dest, context, rpcId, key);
    ...
    if (state == DISCOVERY && dynamic_cast<BeehiveRequestLeafSetCall*>(call)) {
        join();
    }
}


void Beehive::handleBeehiveJoinResponse(BeehiveJoinResponse* response)
{
    EV << "[Beehive::handleBeehiveJoinResponse() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]"
           << endl;
...
void Beehive::handleRequestStateResponse(BeehiveRequestStateResponse* response)
{
    EV << "[Beehive::handleRequestStateResponse() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
...
void Beehive::handleRequestLeafSetResponse(BeehiveRequestLeafSetResponse* response)
{
    EV << "[Beehive::handleRequestLeafSetResponse() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    if (state == DISCOVERY) {
        ...
        discoveryModeProbedNodes = 0;
        BeehiveStateMessage* leaves =
            check_and_cast<BeehiveStateMessage*>(response->getEncapsulatedPacket());
...
void Beehive::handleRequestRoutingRowResponse(BeehiveRequestRoutingRowResponse* response)
{
    EV << "[Beehive::handleRequestRoutingRowResponse() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;

    if (state == DISCOVERY) {
        BeehiveStateMessage* rowState =
            check_and_cast<BeehiveStateMessage*>(response->getEncapsulatedPacket());
...
bool Beehive::recursiveRoutingHook(const TransportAddress& dest,
                                  BaseRouteMessage* msg)
{
    ...
    BeehiveJoinCall* call =
            dynamic_cast<BeehiveJoinCall*>(msg->getEncapsulatedPacket());
...
void Beehive::iterativeJoinHook(BaseOverlayMessage* msg, bool incrHopCount)
{
    BeehiveFindNodeExtData* findNodeExt = NULL;
    if (msg && msg->hasObject("findNodeExt")) {
        findNodeExt =
            check_and_cast<BeehiveFindNodeExtData*>(msg->
...
void Beehive::doJoinUpdate(void)
{
    ...
    if (!stReceived.empty()) {
        for (std::vector<BeehiveStateMsgHandle>::iterator it =
                 stReceived.begin(); it != stReceived.end(); ++it) {
            BeehiveStateMessage* stateMsg =
                 createStateMessage(BEEHIVE_STATE_UPDATE,
                                    it->msg->getTimestamp());
            RECORD_STATS(stateSent++;
                         stateBytesSent += stateMsg->getByteLength());
    ...
    for (std::vector<TransportAddress>::iterator it =
        ...
        if (*it != thisNode) {
            BeehiveStateMessage* stateMsg =
                createStateMessage(BEEHIVE_STATE_JOINUPDATE);
            RECORD_STATS(stateSent++;
                         stateBytesSent += stateMsg->getByteLength());
...
void Beehive::doSecondStage(void)
{
    ...
    if (leafSet->isValid()) {
        ...
        BeehiveStateMsgHandle handle(stateMsg);
        ...
        if (!stateCache.msg) {
            ...
            if (stateCacheQueue.size() > 15) {
                ...
                stateCacheQueue.pop();
                EV << "[Beehive::doSecondStage() @ " << thisNode.getIp()
                   << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    for (std::vector<TransportAddress>::iterator it = notifyList.begin();
        ...
        BeehiveRequestStateCall* call =
            new BeehiveRequestStateCall("REQUEST STATE Call");
        call->setBitLength(BEEHIVEREQUESTREPAIRCALL_L(call));
...
void Beehive::doRoutingTableMaintenance()
{
    for (int i = 0; i < routingTable->getLastRow(); i++) {
        ...
        if ((!ask4row.isUnspecified()) && (ask4row != thisNode)) {
            ...
            EV << "[Beehive::doRoutingTableMaintenance() @ "
               << thisNode.getIp()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
...
bool Beehive::handleFailedNode(const TransportAddress& failed)
{
    ...
    if (failed.isUnspecified())
        opp_error("Beehive::handleFailedNode(): failed is unspecified!");
    ...
    if (!lsAsk.isUnspecified()) {
        ...
        if (sendStateAtLeafsetRepair) {
            BeehiveRequestRepairCall* call =
                new BeehiveRequestRepairCall("REQUEST REPAIR Call");
            call->setBitLength(BEEHIVEREQUESTREPAIRCALL_L(call));
    ...
    if (wasValid && lsAsk.isUnspecified() && (! leafSet->isValid())) {
        EV << "[Beehive::handleFailedNode() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void Beehive::checkProxCache(void)
{
    EV << "[Beehive::checkProxCache() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    if (state == JOIN) {
        ...
        // state == READY
        if (stateCache.msg->getBeehiveStateMsgType() == BEEHIVE_STATE_REPAIR) {
...
void Beehive::endProcessingState(void)
{
    // if state message was not an update, send one back:
    if (stateCache.msg &&
        stateCache.msg->getBeehiveStateMsgType() != BEEHIVE_STATE_UPDATE &&
...
bool Beehive::mergeState(void)
{
    ...
    if (state == JOIN) {
        ...
        if (debugOutput) {
            EV << "[Beehive::mergeState() @ " << thisNode.getIp()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    [JOIN] starting to build own state from "
               << stReceived.size() << " received state messages..."
               << endl;
...
void Beehive::handleStateMessage(BeehiveStateMessage* msg)
{
    if (debugOutput) {
        EV << "[Beehive::handleStateMessage() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    BeehiveStateMsgHandle handle(msg);

    // in JOIN state, store all received state Messages, need them later:
    if (state == JOIN) {
        if (!(msg->getBeehiveStateMsgType() &
              (BEEHIVE_STATE_JOIN | BEEHIVE_STATE_MINJOIN))) {
            delete msg;
            return;
        }
    ...
    if (debugOutput) {
        EV << "[Beehive::handleStateMessage() @ " << thisNode.getIp()
           ...
           << endl;
        EV << "        type: " << ((msg->getBeehiveStateMsgType()
                                    == BEEHIVE_STATE_UPDATE) ? "update"
                                                            :"standard")
           << endl;
        if (msg->getBeehiveStateMsgType() == BEEHIVE_STATE_UPDATE) {
            EV << "        msg timestamp:      " <<
    ...
    if (msg->getBeehiveStateMsgType() == BEEHIVE_STATE_REPAIR) {
        ...
        if (! askLs.isUnspecified()) {
            //sendRequest(askLs, BEEHIVE_REQ_REPAIR);
            BeehiveRequestRepairCall* call =
                new BeehiveRequestRepairCall("REQUEST REPAIR Call");
            call->setBitLength(BEEHIVEREQUESTREPAIRCALL_L(call));
            RECORD_STATS(repairReqSent++; repairReqBytesSent += call->getByteLength());
    ...
    if (!stateCache.msg) {
        ...
        if (proximityNeighborSelection && (pingBeforeSecondStage ||
            msg->getBeehiveStateMsgType() == BEEHIVE_STATE_STD)) {
...
void Beehive::processState(void)
{
    EV << "[Beehive::processState() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]\n"
       << "    new \""
       << std::string(cEnum::find("BeehiveStateMsgType")
              ->getStringFor(stateCache.msg->getBeehiveStateMsgType())).erase(0, 13)
       << "\" STATE message " << static_cast<void*>(stateCache.msg)
       << " from " << stateCache.msg->getSender().getIp() << " to process "
       << endl;

    if (proximityNeighborSelection && (pingBeforeSecondStage ||
        stateCache.msg->getBeehiveStateMsgType() == BEEHIVE_STATE_STD)) {
        pingNodes();
    } else {
        mergeState();
        endProcessingState();
