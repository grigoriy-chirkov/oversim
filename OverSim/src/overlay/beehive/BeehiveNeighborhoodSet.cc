...
/**
 * @file BeehiveNeighborhoodSet.cc
...
Define_Module(BeehiveNeighborhoodSet);


void BeehiveNeighborhoodSet::earlyInit(void)
{
    WATCH_VECTOR(neighbors);
}


void BeehiveNeighborhoodSet::initializeSet(uint32_t numberOfNeighbors,
...
void BeehiveNeighborhoodSet::dumpToStateMessage(BeehiveStateMessage* msg) const
{
    uint32_t i = 0;
    uint32_t size = 0;
    std::vector<BeehiveExtendedNode>::const_iterator it;
...
const NodeHandle& BeehiveNeighborhoodSet::findCloserNode(const OverlayKey& destination,
                                                        bool optimize)
{
    std::vector<BeehiveExtendedNode>::const_iterator it;
...
void BeehiveNeighborhoodSet::findCloserNodes(const OverlayKey& destination,
                                            NodeVector* nodes)
{
    std::vector<BeehiveExtendedNode>::const_iterator it;
...
bool BeehiveNeighborhoodSet::mergeNode(const NodeHandle& node, simtime_t prox)
{
    std::vector<BeehiveExtendedNode>::iterator it;
    ...
    neighbors.insert(it, BeehiveExtendedNode(node, prox)); // insert the entry there
...
void BeehiveNeighborhoodSet::dumpToVector(std::vector<TransportAddress>& affected) const
{
    std::vector<BeehiveExtendedNode>::const_iterator it;
...
const TransportAddress& BeehiveNeighborhoodSet::failedNode(const TransportAddress& failed)
{
    std::vector<BeehiveExtendedNode>::iterator it;
