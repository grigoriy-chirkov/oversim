...
#include "BaseBeehive.h"


void BaseBeehive::purgeVectors(void)
{
...
void BaseBeehive::baseInit()
{
    ...
    if (!neighborCache->isEnabled()) {
        throw cRuntimeError("NeighborCache is disabled, which is mandatory "
                                "for Beehive/Bamboo. Activate it by setting "
                                "\"**.neighborCache.enableNeighborCache "
                                "= true\" in your omnetpp.ini!");
    }

    if (numberOfLeaves % 2) {
        EV << "[BaseBeehive::baseInit() @ " << thisNode.getIp()
    ...
    routingTable = check_and_cast<BeehiveRoutingTable*>
        (getParentModule()->getSubmodule("beehiveRoutingTable"));
    leafSet = check_and_cast<BeehiveLeafSet*>
        (getParentModule()->getSubmodule("beehiveLeafSet"));
    neighborhoodSet = check_and_cast<BeehiveNeighborhoodSet*>
        (getParentModule()->getSubmodule("beehiveNeighborhoodSet"));
...
void BaseBeehive::baseChangeState(int toState)
{
    switch (toState) {
    case INIT:
        state = INIT;
    ...
    case JOIN:
        ...
        if (bootstrapNode.isUnspecified()) {
            // no existing beehive network -> first node of a new one
            changeState(READY);
            return;
        }
...
void BaseBeehive::newLeafs(void)
{
    if (! enableNewLeafs) return;

    BeehiveNewLeafsMessage* msg = leafSet->getNewLeafsMessage();
    if (msg) {
        send(msg, "appOut");
        EV << "[BaseBeehive::newLeafs() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    newLeafs() called."
           << endl;
    }
...
void BaseBeehive::proxCallback(const TransportAddress& node, int rpcId,
                              cPolymorphic *contextPointer, Prox prox)
{
    Enter_Method("proxCallback()");

    EV << "[BaseBeehive::proxCallback() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Pong (or timeout) received (from "
           << node.getIp() << ")"
           << endl;

    double rtt = ((prox == Prox::PROX_TIMEOUT) ? BEEHIVE_PROX_INFINITE
                                               : prox.proximity);
    ...
    if (contextPointer != NULL && stateCache.msg && stateCache.prox) {
        ...
        // handle failed node
        if (rtt == BEEHIVE_PROX_INFINITE && state == READY) {
...
void BaseBeehive::prePing(const BeehiveStateMessage* stateMsg)
{
    ...
    for (uint32_t i = 0; i < rt_size + ls_size + ns_size; i++) {
        ...
        if (!node->isUnspecified()) {
            ...
            if (prox == Prox::PROX_SELF) {
                ...
                *proxPos = BEEHIVE_PROX_INFINITE;
            } else if (prox == Prox::PROX_WAITING) {
                *proxPos = BEEHIVE_PROX_PENDING;
            } else {
...
void BaseBeehive::determineAliveTable(const BeehiveStateMessage* stateMsg)
{
    uint32_t rt_size = stateMsg->getRoutingTableArraySize();
    ...
    for (uint32_t i = 0; i < rt_size + ls_size + ns_size; i++) {
        ...
        if (node->isUnspecified()) {
            *tblPos = BEEHIVE_PROX_UNDEF;
        } else if (neighborCache->getProx(*node,
                                          NEIGHBORCACHE_DEFAULT_IMMEDIATELY) ==
                   Prox::PROX_TIMEOUT) {
            *tblPos = BEEHIVE_PROX_INFINITE;
        }
    }
}


bool BaseBeehive::handleRpcCall(BaseCallMessage* msg)
{
    if (state != READY) {
        EV << "[BaseBeehive::handleRpcCall() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    RPC_DELEGATE( BeehiveRequestLeafSet, handleRequestLeafSetCall );
    RPC_DELEGATE( BeehiveRequestRoutingRow, handleRequestRoutingRowCall );
    RPC_SWITCH_END( )
...
void BaseBeehive::handleRpcResponse(BaseResponseMessage* msg,
                                   cPolymorphic* context, int rpcId,
                                   simtime_t rtt)
{
    RPC_SWITCH_START(msg)
    RPC_ON_RESPONSE( BeehiveRequestLeafSet ) {
        EV << "[BaseBeehive::handleRpcResponse() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Received a RequestLeafSet RPC Response: id=" << rpcId << "\n"
           << "    msg=" << *_BeehiveRequestLeafSetResponse << " rtt=" << SIMTIME_DBL(rtt)
           << endl;
        BaseBeehive::handleRequestLeafSetResponse(_BeehiveRequestLeafSetResponse);
        break;
    }
    RPC_ON_RESPONSE( BeehiveRequestRoutingRow ) {
        EV << "[BaseBeehive::handleRpcResponse() @ " << thisNode.getIp()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Received a RequestRoutingRow RPC Response: id=" << rpcId << "\n"
           << "    msg=" << *_BeehiveRequestRoutingRowResponse << " rtt=" << SIMTIME_DBL(rtt)
           << endl;
        BaseBeehive::handleRequestRoutingRowResponse(_BeehiveRequestRoutingRowResponse);
        break;
    }
    RPC_SWITCH_END( )
}
...
void BaseBeehive::handleRpcTimeout(BaseCallMessage* call,
                                  const TransportAddress& dest,
                                  cPolymorphic* context, int rpcId,
                                  const OverlayKey& key)
{
    EV << "[BaseBeehive::handleRpcTimeout() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void BaseBeehive::handleRequestLeafSetCall(BeehiveRequestLeafSetCall* call)
{
    EV << "[BaseBeehive::handleRequestLeafSetCall() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    BeehiveRequestLeafSetResponse* response =
        new BeehiveRequestLeafSetResponse("REQUEST LEAFSET Response");
    response->setTimestamp(simTime());
    response->setStatType(MAINTENANCE_STAT);

    response->setBitLength(BEEHIVEREQUESTLEAFSETRESPONSE_L(response));
    response->encapsulate(createStateMessage(BEEHIVE_STATE_LEAFSET));
    ...
    if (call->getEncapsulatedPacket()) {
        ...
        BeehiveStateMessage* stateMsg =
            check_and_cast<BeehiveStateMessage*>(call->decapsulate());
...
void BaseBeehive::handleRequestRoutingRowCall(BeehiveRequestRoutingRowCall* call)
{
    EV << "[BaseBeehive::handleRequestRoutingRowCall() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    if (state == READY) {
        ...
        if (call->getRow() > routingTable->getLastRow()) {
            ...
            BeehiveRequestRoutingRowResponse* response =
                new BeehiveRequestRoutingRowResponse("REQUEST ROUTINGROW Response");
            response->setStatType(MAINTENANCE_STAT);

            assert(call->getRow() >= -1 &&
                   call->getRow() <= routingTable->getLastRow());

            response->setBitLength(BEEHIVEREQUESTROUTINGROWRESPONSE_L(response));
            response->encapsulate(createStateMessage(BEEHIVE_STATE_ROUTINGROW,
                                                     -1,
                                                     (call->getRow() == -1) ?
...
void BaseBeehive::handleRequestLeafSetResponse(BeehiveRequestLeafSetResponse* response)
{
    EV << "[BaseBeehive::handleRequestLeafSetResponse() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    if (state == READY) {
        handleStateMessage(check_and_cast<BeehiveStateMessage*>(response->decapsulate()));
    }
}


void BaseBeehive::handleRequestRoutingRowResponse(BeehiveRequestRoutingRowResponse* response)
{
    EV << "[BaseBeehive::handleRequestRoutingRowResponse() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
...
BeehiveStateMessage* BaseBeehive::createStateMessage(enum BeehiveStateMsgType type,
                                                   simtime_t timestamp,
                                                   int16_t row,
                                                   bool lastHop)
{
    //checks
    if ((type & (BEEHIVE_STATE_JOIN | BEEHIVE_STATE_MINJOIN)) &&
        ((row == -1) || (timestamp != -1))) {
        throw cRuntimeError("Creating JOIN State without hopCount"
                            " or with timestamp!");
    } else if ((type & (BEEHIVE_STATE_ROUTINGROW)) && row == -1) {
        throw cRuntimeError("Creating ROUTINGROW State without row!");
    } else if ((type & (BEEHIVE_STATE_UPDATE)) &&
                timestamp == -1) {
        throw cRuntimeError("Creating UPDATE/REPAIR State without timestamp!");
    } else if ((type & (BEEHIVE_STATE_LEAFSET | BEEHIVE_STATE_STD |
                        BEEHIVE_STATE_REPAIR  | BEEHIVE_STATE_JOINUPDATE)) &&
            ((row != -1) || (timestamp != -1))) {
            throw cRuntimeError("Creating STD/UPDATE State with row/hopCount"
                                " or timestamp!");
    }

    std::string typeStr = cEnum::get("BeehiveStateMsgType")->getStringFor(type);
    typeStr.erase(0, 13);

    EV << "[BaseBeehive::sendStateTables() @ " << thisNode.getIp()
       << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    BeehiveStateMessage* stateMsg =
        new BeehiveStateMessage((std::string("STATE (") + typeStr + ")").c_str());

    stateMsg->setTimestamp((timestamp == -1) ? simTime() : timestamp);

    if (type == BEEHIVE_STATE_JOIN || type == BEEHIVE_STATE_MINJOIN) {
        stateMsg->setRow(row);
    ...
    stateMsg->setBeehiveStateMsgType(type);
    ...
    } else if (type == BEEHIVE_STATE_LEAFSET) {
        leafSet->dumpToStateMessage(stateMsg);
    } else if (type == BEEHIVE_STATE_ROUTINGROW) {
        routingTable->dumpRowToMessage(stateMsg, row);
    } else {
    ...
    stateMsg->setBitLength(BEEHIVESTATE_L(stateMsg));

    return stateMsg;
}


bool BaseBeehive::isSiblingFor(const NodeHandle& node,
                              const OverlayKey& key,
                              int numSiblings,
                              bool* err)
{
...
void BaseBeehive::handleAppMessage(BaseOverlayMessage* msg)
{
    delete msg;
}


void BaseBeehive::updateTooltip()
{
    if (ev.isGUI()) {
...
BaseBeehive::~BaseBeehive()
{
    purgeVectors();
}

void BaseBeehive::finishOverlay()
{
    ...
    if (globalStatistics->getMeasureNetwInitPhase() ||
        ...
        if (joinTries > 0) {
            globalStatistics->addStdDev("BaseBeehive: join success ratio", (double)joins / (double)joinTries);
            globalStatistics->addStdDev("BaseBeehive: join tries", joinTries);
        }
    }

    // TODO new methods Beehive::finnishOverlay() / Bamboo::finishOverlay()
    globalStatistics->addStdDev("Beehive: joins with missing replies from routing path/s",
                                joinPartial / time);
    globalStatistics->addStdDev("Beehive: JOIN Messages seen/s", joinSeen / time);
    globalStatistics->addStdDev("Beehive: bytes of JOIN Messages seen/s", joinBytesSeen / time);
    globalStatistics->addStdDev("Beehive: JOIN Messages received/s", joinReceived / time);
    globalStatistics->addStdDev("Beehive: bytes of JOIN Messages received/s",
                                joinBytesReceived / time);
    globalStatistics->addStdDev("Beehive: JOIN Messages sent/s", joinSent / time);
    globalStatistics->addStdDev("Beehive: bytes of JOIN Messages sent/s", joinBytesSent / time);

    globalStatistics->addStdDev("Beehive: REPAIR Requests sent/s", repairReqSent / time);
    globalStatistics->addStdDev("Beehive: bytes of REPAIR Requests sent/s",
                                repairReqBytesSent / time);
    globalStatistics->addStdDev("Beehive: REPAIR Requests received/s", repairReqReceived / time);
    globalStatistics->addStdDev("Beehive: bytes of REPAIR Requests received/s",
                                repairReqBytesReceived / time);

    globalStatistics->addStdDev("Beehive: STATE Requests sent/s", stateReqSent / time);
    globalStatistics->addStdDev("Beehive: bytes of STATE Requests sent/s", stateReqBytesSent / time);
    globalStatistics->addStdDev("Beehive: STATE Requests received/s", stateReqReceived / time);
    globalStatistics->addStdDev("Beehive: bytes of STATE Requests received/s",
                                stateReqBytesReceived / time);
    globalStatistics->addStdDev("Beehive: STATE Messages sent/s", stateSent / time);
    globalStatistics->addStdDev("Beehive: bytes of STATE Messages sent/s", stateBytesSent / time);
    globalStatistics->addStdDev("Beehive: STATE Messages received/s", stateReceived / time);
    globalStatistics->addStdDev("Beehive: bytes of STATE Messages received/s",
                                stateBytesReceived / time);

    globalStatistics->addStdDev("BaseBeehive: LEAFSET Requests sent/s", leafsetReqSent / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of LEAFSET Requests sent/s", leafsetReqBytesSent / time);
    globalStatistics->addStdDev("BaseBeehive: LEAFSET Requests received/s", leafsetReqReceived / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of LEAFSET Requests received/s",
                                leafsetReqBytesReceived / time);
    globalStatistics->addStdDev("BaseBeehive: LEAFSET Messages sent/s", leafsetSent / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of LEAFSET Messages sent/s", leafsetBytesSent / time);
    globalStatistics->addStdDev("BaseBeehive: LEAFSET Messages received/s", leafsetReceived / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of LEAFSET Messages received/s",
                                leafsetBytesReceived / time);

    globalStatistics->addStdDev("BaseBeehive: ROUTING TABLE ROW Requests sent/s", routingTableRowReqSent / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of ROUTING TABLE ROW Requests sent/s", routingTableRowReqBytesSent / time);
    globalStatistics->addStdDev("BaseBeehive: ROUTING TABLE ROW Requests received/s", routingTableRowReqReceived / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of ROUTING TABLE ROW Requests received/s",
                                routingTableRowReqBytesReceived / time);
    globalStatistics->addStdDev("BaseBeehive: ROUTING TABLE ROW Messages sent/s", routingTableRowSent / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of ROUTING TABLE ROW Messages sent/s", routingTableRowBytesSent / time);
    globalStatistics->addStdDev("BaseBeehive: ROUTING TABLE ROW Messages received/s", routingTableRowReceived / time);
    globalStatistics->addStdDev("BaseBeehive: bytes of ROUTING TABLE ROW Messages received/s",
                                routingTableRowBytesReceived / time);

    globalStatistics->addStdDev("BaseBeehive: total number of lookups", totalLookups);
    globalStatistics->addStdDev("BaseBeehive: responsible lookups", responsibleLookups);
    globalStatistics->addStdDev("BaseBeehive: lookups in routing table", routingTableLookups);
    globalStatistics->addStdDev("BaseBeehive: lookups using closerNode()", closerNodeLookups);
    globalStatistics->addStdDev("BaseBeehive: lookups using closerNode() with result from "
                                "neighborhood set", closerNodeLookupsFromNeighborhood);
}


int BaseBeehive::getMaxNumSiblings()
{
    return (int)floor(numberOfLeaves / 2.0);
}


int BaseBeehive::getMaxNumRedundantNodes()
{
    return (int)floor(numberOfLeaves);
}


NodeVector* BaseBeehive::findNode(const OverlayKey& key,
                                 ...
                                 BaseOverlayMessage* msg)
{
    if ((numRedundantNodes > getMaxNumRedundantNodes()) ||
        ...
        opp_error("(Beehive::findNode()) numRedundantNodes or numSiblings "
                  "too big!");
    }
...
AbstractLookup* BaseBeehive::createLookup(RoutingType routingType,
                                         ...
                                         bool appLookup)
{
    ...
    BeehiveFindNodeExtData* findNodeExt =
        new BeehiveFindNodeExtData("findNodeExt");

    if (msg &&
        dynamic_cast<const BeehiveJoinCall*>(msg->getEncapsulatedPacket())) {
        const BeehiveJoinCall* joinCall =
            static_cast<const BeehiveJoinCall*>(msg->getEncapsulatedPacket());
        assert(!joinCall->getSrcNode().isUnspecified());
        findNodeExt->setSendStateTo(joinCall->getSrcNode());
        findNodeExt->setJoinHopCount(1);
    }
    findNodeExt->setBitLength(BEEHIVEFINDNODEEXTDATA_L);
...
bool stateMsgIsSmaller(const BeehiveStateMsgHandle& hnd1,
                       const BeehiveStateMsgHandle& hnd2)
{
    return (hnd1.msg->getRow() < hnd2.msg->getRow());
}


uint8_t BaseBeehive::getRTLastRow() const
{
    return routingTable->getLastRow();
};


std::vector<TransportAddress>* BaseBeehive::getRTRow(uint8_t index) const
{
    return routingTable->getRow(index);
};


std::vector<TransportAddress>* BaseBeehive::getLeafSet() const
{
...
std::ostream& operator<<(std::ostream& os, const BeehiveStateMsgProximity& pr)
{
    os << "BeehiveStateMsgProximity {" << endl;
    os << "  pr_rt {" << endl;
...
OverlayKey BaseBeehive::estimateMeanDistance()
{
...
OverlayKey BaseBeehive::distance(const OverlayKey& x,
                                const OverlayKey& y,
