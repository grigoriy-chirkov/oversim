...
#ifndef __BASEBEEHIVE_H_
#define __BASEBEEHIVE_H_
...
#include "BeehiveNeighborhoodSet.h"

class BeehiveLeafSet;


class BaseBeehive : public BaseOverlay, public ProxListener
{
public:
     ...
     *
     * @param msg the beehive state message
     */
    virtual void handleStateMessage(BeehiveStateMessage* msg) = 0;
  ...
  protected:
    ...
    void handleRequestLeafSetCall(BeehiveRequestLeafSetCall* call);

    void handleRequestRoutingRowCall(BeehiveRequestRoutingRowCall* call);

    virtual void handleRequestLeafSetResponse(BeehiveRequestLeafSetResponse* response);

    virtual void handleRequestRoutingRowResponse(BeehiveRequestRoutingRowResponse* response);

    BeehiveStateMessage* createStateMessage(enum BeehiveStateMsgType type = BEEHIVE_STATE_STD,
    ...
    BeehiveRoutingTable* routingTable;
    BeehiveLeafSet* leafSet;
    BeehiveNeighborhoodSet* neighborhoodSet;
    ...
    /**
     * initializes parameters and variables used in both Bamboo and Beehive
     */
    void baseInit(void);
     ...
     */
    void prePing(const BeehiveStateMessage* stateMsg);

    /**
     * ping all nodes in the beehive state message pointed to by
     ...
     * value of BEEHIVE_PROX_INFINITE, all other nodes just get a
     * value of 1
     */
    void determineAliveTable(const BeehiveStateMessage* stateMsg);

    /**
     * Beehive API: send newLeafs() to application if enabled
     */
    void newLeafs(void);
  ...
  public:
    ...
    friend class BeehiveLeafSet;
};

/**
 * predicate for comparing two pointers to BeehiveStateMessages based on their
 * joinHopCount. Needed for sorting the received BeehiveStateMessages.
 */
