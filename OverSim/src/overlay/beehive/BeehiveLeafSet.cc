...
Define_Module(BeehiveLeafSet);


void BeehiveLeafSet::earlyInit()
{
    WATCH_VECTOR(leaves);
}


void BeehiveLeafSet::initializeSet(uint32_t numberOfLeaves,
...
const NodeHandle& BeehiveLeafSet::getSuccessor() const
{
    return *bigger;
}


const NodeHandle& BeehiveLeafSet::getPredecessor() const
{
    return *smaller;
}


bool BeehiveLeafSet::isValid() const
{
    return (!(smaller->isUnspecified() || bigger->isUnspecified()));
}


const NodeHandle& BeehiveLeafSet::getDestinationNode(const OverlayKey& destination)
{
...
bool BeehiveLeafSet::isClosestNode(const OverlayKey& destination) const
{
...
void BeehiveLeafSet::dumpToStateMessage(BeehiveStateMessage* msg) const
{
    uint32_t i = 0;
    uint32_t size = 0;
...
const TransportAddress& BeehiveLeafSet::getRandomNode()
{
    uint32_t rnd;
    int i;
...
NodeVector* BeehiveLeafSet::createSiblingVector(const OverlayKey& key,
...
bool BeehiveLeafSet::mergeNode(const NodeHandle& node, simtime_t prox)
{
...
void BeehiveLeafSet::insertLeaf(std::vector<NodeHandle>::iterator& it,
    ...
    if (issmaller) {
        ...
        if (!leaves.front().isUnspecified())  {
            ...
            if (leaves.back().isUnspecified()) {
                ...
                EV << "[BeehiveLeafSet::insertLeaf() @ " << overlay->getThisNode().getIp()
...
bool BeehiveLeafSet::balanceLeafSet()
{
    if (isFull ||
...
void BeehiveLeafSet::dumpToVector(std::vector<TransportAddress>& affected) const
{
    std::vector<NodeHandle>::const_iterator it;
...
const NodeHandle& BeehiveLeafSet::getSmallestNode(void) const
{
...
const OverlayKey& BeehiveLeafSet::getSmallestKey(void) const
{
    return getSmallestNode().getKey();
}


const NodeHandle& BeehiveLeafSet::getBiggestNode(void) const
{
...
const OverlayKey& BeehiveLeafSet::getBiggestKey(void) const
{
    return getBiggestNode().getKey();
}


void BeehiveLeafSet::findCloserNodes(const OverlayKey& destination,
                                    NodeVector* nodes)
{
    std::vector<NodeHandle>::const_iterator it;
...
const NodeHandle& BeehiveLeafSet::findCloserNode(const OverlayKey& destination,
                                                bool optimize)
{
    std::vector<NodeHandle>::const_iterator i;
    const NodeHandle* ret = &NodeHandle::UNSPECIFIED_NODE;
...
const TransportAddress& BeehiveLeafSet::failedNode(const TransportAddress& failed)
{
...
const TransportAddress& BeehiveLeafSet::repair(const BeehiveStateMessage* msg,
...
BeehiveNewLeafsMessage* BeehiveLeafSet::getNewLeafsMessage(void)
{
    std::vector<NodeHandle>::const_iterator it;
    BeehiveNewLeafsMessage* msg;
    uint32_t i = 0;

    if (! newLeafs) return NULL;
    newLeafs = false;

    msg = new BeehiveNewLeafsMessage("BeehiveNewLeafs");

    msg->setLeafsArraySize(numberOfLeaves);
    for (it = leaves.begin(); it != leaves.end(); it++)
        msg->setLeafs(i++, *it);

    msg->setBitLength(BEEHIVENEWLEAFS_L(msg));
    return msg;
}


OverlayKey BeehiveLeafSet::estimateMeanDistance()
{
...
bool BeehiveLeafSet::mergeState(const BeehiveStateMessage* msg,
                               const BeehiveStateMsgProximity* prox)
{
    // LS to temp vector
    std::vector<NodeHandle> temp = leaves; //getNodeHandleVector();

    bool result = BeehiveStateObject::mergeState(msg, prox);
