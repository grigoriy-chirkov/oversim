#ifndef __BEEHIVEROUTINGTABLE_H_
#define __BEEHIVEROUTINGTABLE_H_
...
/**
 * Vector-type of a line in Beehive IRoutingTable
 */
typedef std::vector<BeehiveExtendedNode> PRTRow;
 ...
 * @see Beehive
 */
class BeehiveRoutingTable : public BeehiveStateObject
{
  ...
  public:
     ...
     *
     * @param bitsPerDigit Beehive configuration parameter
     * @param repairTimeout Beehive configuration parameter
    ...
    /**
     * gets the next hop according to the Beehive routing scheme.
     *
     ...
     */
    virtual const TransportAddress& repair(const BeehiveStateMessage* msg,
                                           const BeehiveStateMsgProximity* prox);

    /**
     * dump content of the table to a BeehiveStateMessage
     *
     ...
     */
    virtual void dumpRowToMessage(BeehiveStateMessage* msg, int row) const;
    ...
    /**
     * initialize table from vector of BeehiveStateMsgHandles with STATE
     * messages received during JOIN phase. The vector has to be sorted by
     * JoinHopCount of the messages
     *
     * @param handles the vector of BeehiveStateMsgHandles
     * @return true on success
     */
    bool initStateFromHandleVector(const std::vector<BeehiveStateMsgHandle>& handles);
  ...
  private:
     ...
     */
    const BeehiveExtendedNode& nodeAt(uint32_t row, uint32_t col) const;
