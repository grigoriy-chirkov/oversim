...
/**
 * @file BeehiveStateObject.h
 * @author Grigory Chirkov
 * Based on the Pastry overlay implementation
 */


#ifndef __BEEHIVESTATEOBJECT_H
#define __BEEHIVESTATEOBJECT_H
...
/**
 * BeehiveStateObject Module
 *
 * This module class describes the common interface of all Beehive State Objects
 * and implements what all have in common
 *
 * @author Felix Palmen
 * @see BeehiveRoutingTable, LeafSet, NeighborhoodSet
 */
class BeehiveStateObject : public cSimpleModule
{
  public:
    ...
    /**
     * gets the final node according to the Beehive routing scheme.
     *
     ...
     */
    virtual void dumpToStateMessage(BeehiveStateMessage* msg) const = 0;

    /**
     * update own state based on a received BeehiveStateMessage
     *
     * @param msg the BeehiveStateMessage to use as source for update
     * @param prox record of proximity values matching the state message
     * @return true if leafSet was actually changed
     */
    virtual bool mergeState(const BeehiveStateMessage* msg,
                            const BeehiveStateMsgProximity* prox);
     ...
     *
     * This is needed for the "rare case" in the Beehive routing algorithm.
     *
     * @param test the NodeHandle to test
     * @param destination the destination Key
     * @param reference NodeHandle to compare to, own node if unset
  ...
  protected:
    ...
    /**
     * stores the NodeHandle of the owner of this BeehiveStateObject.
     ...
     */
  private:
    static const BeehiveExtendedNode* _unspecNode;

  protected:
    static const BeehiveExtendedNode& unspecNode()
    {
        if (_unspecNode == NULL)
            _unspecNode = new BeehiveExtendedNode();
        return *_unspecNode;
    }
