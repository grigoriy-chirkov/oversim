...
Define_Module(BeehiveFingerTable);

void BeehiveFingerTable::initialize(int stage)
{
...
void BeehiveFingerTable::handleMessage(cMessage* msg)
{
    error("this module doesn't handle messages, it runs only in initialize()");
}

void BeehiveFingerTable::initializeTable(uint32_t size, const NodeHandle& owner,
                                       Beehive* overlay)
{
    maxSize = size;
...
uint32_t BeehiveFingerTable::getSize()
{
    return maxSize;
}

void BeehiveFingerTable::setFinger(uint32_t pos, const NodeHandle& node,
                                 Successors const* sucNodes)
{
    if (pos >= maxSize) {
        throw new cRuntimeError("BeehiveFingerTable::setFinger(): "
                                "Index out of bound");
    }
...
void BeehiveFingerTable::setFinger(uint32_t pos, const Successors& nodes)
{
    setFinger(pos, nodes.begin()->second, &nodes);
}

bool BeehiveFingerTable::updateFinger(uint32_t pos, const NodeHandle& node,
                                    simtime_t rtt)
{
    if (rtt < 0)
        return false;
    ...
    if (pos >= maxSize) {
         throw new cRuntimeError("BeehiveFingerTable::updateFinger(): "
                                 "Index out of bound");
     }
...
bool BeehiveFingerTable::handleFailedNode(const TransportAddress& failed)
{
    bool ret = false;
...
void BeehiveFingerTable::removeFinger(uint32_t pos)
{
    if (pos >= maxSize) {
        throw new cRuntimeError("BeehiveFingerTable::removeFinger(): "
                                "Index out of bound");
    }
...
const NodeHandle& BeehiveFingerTable::getFinger(uint32_t pos)
{
    if (pos >= maxSize) {
        throw new cRuntimeError("BeehiveFingerTable::getFinger(): "
                                "Index out of bound");
    }
...
NodeVector* BeehiveFingerTable::getFinger(uint32_t pos, const OverlayKey& key)
{
    if (pos >= maxSize) {
        throw new cRuntimeError("BeehiveFingerTable::getFinger(): "
                                "Index out of bound");
