...
Define_Module(BeehiveSuccessorList);
...
*/
void BeehiveSuccessorList::initialize(int stage)
{
...
void BeehiveSuccessorList::handleMessage(cMessage* msg)
{
    error("this module doesn't handle messages, it runs only in initialize()");
}

void BeehiveSuccessorList::initializeList(uint32_t size, NodeHandle owner,
                                        Beehive *overlay)
{
    successorMap.clear();
...
uint32_t BeehiveSuccessorList::getSize()
{
    return successorMap.size();
}

bool BeehiveSuccessorList::isEmpty()
{
...
const NodeHandle& BeehiveSuccessorList::getSuccessor(uint32_t pos)
{
    // check boundaries
    ...
    if (pos >= successorMap.size()) {
        error("Index out of bound (BeehiveSuccessorList, getSuccessor())");
    }
...
void BeehiveSuccessorList::updateList(BeehiveNotifyResponse* notifyResponse)
{
...
void BeehiveSuccessorList::addSuccessor(NodeHandle successor, bool resize)
{
...
bool BeehiveSuccessorList::handleFailedNode(const TransportAddress& failed)
{
...
void BeehiveSuccessorList::removeOldSuccessors()
{
    std::map<OverlayKey,SuccessorListEntry>::iterator it;
...
void BeehiveSuccessorList::updateDisplayString()
{
...
void BeehiveSuccessorList::updateTooltip()
{
    if (ev.isGUI()) {
...
void BeehiveSuccessorList::display()
{
    cout << "Content of BeehiveSuccessorList:" << endl;
