...
/**
 * @file BeehiveDHTDataStorage.h
 * @author Ingmar Baumgart
 */

#ifndef __BEEHIVEDHTDATASTORAGE_H_
#define __BEEHIVEDHTDATASTORAGE_H_
...
/**
 * BeehiveDHT data storage module
 *
 * This modul contains the data storage of the BeehiveDHT implementation.
 *
 * @author Ingmar Baumgart
 * @see BeehiveDHT
 */
...
struct BeehiveDHTDataEntry
{
    ...
    friend std::ostream& operator<<(std::ostream& Stream, const BeehiveDHTDataEntry entry);
    SiblingVoteMap siblingVote;
};

typedef std::vector<std::pair<OverlayKey, BeehiveDHTDataEntry> > BeehiveDHTDataVector;
typedef std::vector<DhtDumpEntry> BeehiveDHTDumpVector;
typedef std::multimap<OverlayKey, BeehiveDHTDataEntry> BeehiveDHTDataMap;

class BeehiveDHTDataStorage : public cSimpleModule
{
  public:
     ...
     */
    BeehiveDHTDataEntry* getDataEntry(const OverlayKey& key,
     ...
     */
    virtual BeehiveDHTDataVector* getDataVector(const OverlayKey& key,
                                         uint32_t kind = 0,
                                         uint32_t id = 0);
     ...
     * @return An iterator
     */
    virtual const BeehiveDHTDataMap::iterator begin();
     ...
     * @return An iterator
     */
    virtual const BeehiveDHTDataMap::iterator end();
     ...
     * @param responsible
     */
    virtual BeehiveDHTDataEntry* addData(const OverlayKey& key, uint32_t kind,
                                  uint32_t id,
     ...
     */
    BeehiveDHTDumpVector* dumpDht(const OverlayKey& key = OverlayKey::UNSPECIFIED_KEY,
                           uint32_t kind = 0, uint32_t id = 0);

  protected:
    BeehiveDHTDataMap dataMap; /**< internal representation of the data storage */
